...
save_path = os.path.join(appdata_path, 'YourAppFolder', "CN")
...
async def process_server_cmd(ctx: CommonContext, args: dict):
    ...
    if cmd == 'RoomInfo':
        ...
        if start_index == 0:
            ctx.items_received = []
            giveitem("char25unlocked")
            ctx.items_received.append("Trash and The Gang")
        ...
        if start_index == len(ctx.items_received):
            checkandgiveitem("Freddy Fazbear", "char1unlocked", ctx.items_received)
            checkandgiveitem("Bonnie", "char2unlocked", ctx.items_received)
            checkandgiveitem("Chica", "char3unlocked", ctx.items_received)
            checkandgiveitem("Foxy", "char4unlocked", ctx.items_received)
            checkandgiveitem("Freddy Fazbear", "char1unlocked", ctx.items_received)
            checkandgiveitem("Freddy Fazbear", "char1unlocked", ctx.items_received)
            checkandgiveitem("Freddy Fazbear", "char1unlocked", ctx.items_received)
            checkandgiveitem("Freddy Fazbear", "char1unlocked", ctx.items_received)
...
def giveitem(inivalue):
    if os.path.isfile(save_path):
        # Load the existing INI file
        config = configparser.ConfigParser()
        config.read(save_path)
        config.set('CN', inivalue, '1')

def checkandgiveitem(itemname, inivalue, ctx):
    if itemname in ctx:
     if os.path.isfile(save_path):
        # Load the existing INI file
        config = configparser.ConfigParser()
